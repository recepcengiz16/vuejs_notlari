            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" v-model="selectedProduct" @change="productSelected">
                        <option :disabled="product.count==0" v-for="product in getProducts" :key="product.key" :value="product.key">{{product.title}}</option>
...
export default {
    ...
    beforeRouteLeave (to, from, next) {
        if((this.selectedProduct !==null || this.product_count>0 ) && !this.saveButtonClicked ) {
               if(confirm("Kaydedşmemiş değişiklikler var. Yine de çıkmak istiyor musunuz?")){
                    next();
               }else{
                    next(false);
               }

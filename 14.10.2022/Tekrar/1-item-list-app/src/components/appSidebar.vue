<template>
     <aside class="bg-gray-700 w-[280px] h-screen">
      <h3 class="text-2xl font-bold mt-2 p-2">Fatura Listesi</h3>
      <div v-for="(invoice,index) in invoices" :key="invoice.id" class="odd:bg-gray-600 flex justify-between items-center p-2">
        <span>#{{ index +1 }}</span>
...
<script setup>
const props=defineProps({invoices:Array,editInvoice:Function});
</script>